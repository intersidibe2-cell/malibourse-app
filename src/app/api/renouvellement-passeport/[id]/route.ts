...
export async function PUT(
  ...
  try {
    ...
    values.push(id);
    const sql = `UPDATE renouvellements_passeport SET ${sets.join(", ")}, updated_at = NOW() WHERE id = $${idx} RETURNING *`;
    ...
    console.error("PUT renouvellement error:", error);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const result = await query("DELETE FROM renouvellements_passeport WHERE id = $1 RETURNING *", [id]);
    ...
    console.error("DELETE renouvellement error:", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
